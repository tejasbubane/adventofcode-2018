...
specs = describe "Day 1" $ do
  ...
  describe "part 2 - first duplicate frequency" $ do
    it "works for example inputs" $ do
      duplicate [1, -2, 3, 1] `shouldBe` 2
      duplicate [1, -1] `shouldBe` 0
      duplicate [3, 3, 4, -1, -4] `shouldBe` 10
      duplicate [-6, 3, 8, 5, -6] `shouldBe` 5
      duplicate [7, 7, -2, -7, -4] `shouldBe` 14
    it "works for puzzle input" $ do
      duplicate puzzleInput `shouldBe` 790
